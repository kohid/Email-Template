    ...
    TDTEmailTemplateBuilder.prototype = {
        ...
        initializeCanvas: function() {
            ...
            $('#email-canvas').sortable({
                ...
                tolerance: 'pointer',
                items: '> .canvas-widget',
                connectWith: '.tdt-widget-container-content',
                start: function(e, ui) {
            ...
            $('#email-canvas .canvas-widget[data-widget-type="container"]').each(function() {
        ...
        endDrag: function(e) {
            ...
            if ((e.clientX >= canvasRect.left && e.clientX <= canvasRect.right &&
                e.clientY >= canvasRect.top && e.clientY <= canvasRect.bottom) || $targetContainer) {
                
                var widgetType = this.draggedWidget.data('widget');
                this.addWidgetToCanvas(widgetType, e, $targetContainer);
            }
         ...
         */
        addWidgetToCanvas: function(widgetType, e, $targetContainer) {
            var self = this;
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $newWidget.attr('data-widget-type', widgetType);
                        $newWidget.attr('data-widget-name', response.data.type);
                        $newWidget.addClass('canvas-widget');
                        
                        // Remove placeholder if exists
                        $('#email-canvas .canvas-placeholder').remove();
                        
                        // Add to canvas or container
                        if ($targetContainer && $targetContainer.length) {
                            // Add to container
                            $targetContainer.find('.tdt-widget-container-content').first().append($newWidget);
                        } else {
                            // Add to main canvas
                            $('#email-canvas').append($newWidget);
                        }
                        
                        // If it's a container, make it droppable
                        if (widgetType === 'container') {
                            self.initializeContainer($newWidget);
                        }
        ...
        /**
         * Initialize container as droppable
         */
        initializeContainer: function($container) {
            var self = this;
            var $containerContent = $container.find('.tdt-widget-container-content');
            ...
            $containerContent.droppable({
                accept: '.tdt-email-template-widget-card, .canvas-widget',
                tolerance: 'pointer',
                hoverClass: 'container-drag-over',
                drop: function(event, ui) {
                    var $draggedItem = ui.draggable;
                    
                    // Check if it's a widget card from the panel
                    if ($draggedItem.hasClass('tdt-email-template-widget-card')) {
                        var widgetType = $draggedItem.data('widget');
                        self.addWidgetToCanvas(widgetType, event, $container);
            ...
            // Make container content sortable
            $containerContent.sortable({
                placeholder: 'drop-zone',
                tolerance: 'pointer',
                connectWith: '.tdt-widget-container-content, #email-canvas',
                start: function(e, ui) {
                    ui.placeholder.height(ui.item.height());
                },
                update: function() {
